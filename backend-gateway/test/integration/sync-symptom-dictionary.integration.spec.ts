...
describe("SyncSymptomDictionaryUseCase (Integration)", () => {
	...
	beforeAll(async () => {
		...
		useCase = module.get<SyncSymptomDictionaryUseCase>(
			SyncSymptomDictionaryUseCase,
		);
	...
	it("should sync symptoms from diagnostic-engine gRPC and persist in cache", async () => {
		console.log(
			"--- Starting Integration Test: SyncSymptomDictionaryUseCase ---",
		);
		...
		const response = await firstValueFrom(
			diagnosticClient.getAppSymptoms("pt-BR"),
		);
		...
		for (const symptom of sample) {
			...
			console.log(
				`[${symptom.cui}] ${symptom.clinical_name.padEnd(30)} -> ${symptom.layman_term}`,
			);
