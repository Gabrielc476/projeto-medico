...
export class TriageSession {
	private constructor(
		private props: TriageSessionProps,
		private _id: string,
	) {}
	...
	public transition(
		...
		if (!snapshot.can({ type: eventName })) {
			throw new Error(
				`Invalid transition: Cannot send ${eventName} in state ${this.props.status}`,
			);
		}
	...
	public addContextualFactors(factors: string[]): void {
		this.props.contextualFactors = Array.from(
			new Set([...this.props.contextualFactors, ...factors]),
		);
		this.props.updatedAt = new Date();
	}

	public setSymptoms(symptoms: string[]): void {
		this.props.symptoms = Array.from(
			new Set([...this.props.symptoms, ...symptoms]),
		);
		this.props.updatedAt = new Date();
	}
