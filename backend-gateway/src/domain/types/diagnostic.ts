...
export interface DiagnosticService {
	getAppSymptoms(request: {
		language?: string;
	}): Observable<AppSymptomResponse>;
	assessSymptoms(request: {
		symptoms: ExtractedFeature[];
		contextual_factors: ExtractedFeature[];
	}): Observable<SymptomAssessmentResponse>;
	extractContext(request: {
		free_text: string;
	}): Observable<ContextExtractionResponse>;
	extractExam(request: {
		pdf_content: Buffer;
	}): Observable<ContextExtractionResponse>;
}
