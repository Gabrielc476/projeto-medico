...
export class AuditLogInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const isClinicalAction = url.includes('/triage') || url.includes('/symptoms') || url.includes('/auth');
    ...
    return next.handle().pipe(
      tap(async (response) => {
        ...
        await this.logAction(method, url, body, user, response, duration, 'SUCCESS');
      }),
      catchError((error) => {
        const duration = Date.now() - startTime;
        this.logAction(method, url, body, user, null, duration, 'FAILURE', error.message);
