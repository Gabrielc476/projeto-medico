...
import { type ExtractedFeature, type ContextExtractionResponse } from '../../domain/types/diagnostic';
...
export class ExtractExamUseCase {
  ...
  async execute(sessionId: string, pdfBuffer: Buffer, examUrl: string): Promise<ExtractedFeature[]> {
    ...
    try {
      ...
      const mergedFactors = Array.from(new Set([...existingFactors, ...newFactors]));

      session.transition('UPLOAD_EXAM');
      await this.triageRepository.updateSessionContext(sessionId, mergedFactors);
